...
//                Log.e("bleDiscovery", name + "|" + mac +"|"+ rssi)
    ...
    private var connectCallback: (ok: Boolean, errCode: Int, errMsg: String) -> Unit =
        { _, _, _ -> }
    private var ecBLECharacteristicValueChangeCallback: (str: String, hexStr: String, characteristic: BluetoothGattCharacteristic) -> Unit =
        { _, _, _ -> }

    fun onBLECharacteristicValueChange(callback: (str: String, hexStr: String, characteristic: BluetoothGattCharacteristic) -> Unit) {
        ecBLECharacteristicValueChangeCallback = callback
    }

    private const val ecCharacteristicWriteUUID = "6e400002-b5a3-f393-e0a9-e50e24dcca9e"
    private const val ecCharacteristicNotifyUUID = "6e400003-b5a3-f393-e0a9-e50e24dcca9e"
    private const val ecATCharacteristicUUID = "6e400004-b5a3-f393-e0a9-e50e24dcca9e"
    private var ecCharacteristicWrite: BluetoothGattCharacteristic? = null
    private var ecATCharacteristicWrite: BluetoothGattCharacteristic? = null
                ...
                try {
                    for (service in bluetoothGattServices) {
                        ...
                        for (characteristic in listGattCharacteristic) {
                            Log.d("ble-char", "UUID=:" + characteristic.uuid.toString())
                            //notify
...
//                            }
                            ...
                            //write
                            else if (characteristic.uuid.toString() == ecCharacteristicWriteUUID) {
                                ecCharacteristicWrite = characteristic
                            }
                            //AT UUID
                            //notify
                            else if (characteristic.uuid.toString() == ecATCharacteristicUUID) {
                                notifyBLECharacteristicValueChange(characteristic)
                                ecATCharacteristicWrite = characteristic
                                ECBLE.writeATBLECharacteristicValue("AT+CNT_LIST\r\n", false)
                            }
        ...
        override fun onCharacteristicChanged(
            ...
            if (ecBLEChineseType == ECBLEChineseTypeGBK) {
                try {
                    str = String(value, charset("GBK"))
                } catch (ignored: Throwable) {
                }
            } else {
                str = String(value)
            }
            val strHex: String = bytesToHexString(value)
            Log.e("ble-receive", "读取成功[string]:$str")
            Log.e("ble-receive", "读取成功[hex]:$strHex")
            Log.d("ble-receive", "UUID=:" + characteristic.uuid.toString())
            ecBLECharacteristicValueChangeCallback(str, strHex, characteristic)
        }
    ...
    private fun notifyBLECharacteristicValueChange(characteristic: BluetoothGattCharacteristic) {
        ...
        if (!res) {
            Log.e("notify err",characteristic.uuid.toString())
            return
        }
...
//            dp.setValue()
        ...
        var ret = bluetoothGatt?.writeDescriptor(descriptor,BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE)!!
        Log.d("writeDescriptor", "$ret ${characteristic.uuid}")
        while (ret != 0) {

            ret = bluetoothGatt?.writeDescriptor(descriptor,BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE)!!
            Log.d("writeDescriptor", "$ret ${characteristic.uuid}")
            Thread.sleep(1000)
        }
    }
    ...
    private fun _createBLEConnection(ctx: Context, id: String) {
        ...
        for (tempDevice in deviceList) {
            if (tempDevice.getAddress().replace(":", "") == id) {
                tempDevice.setPin("135648".toByteArray())
    ...
    @SuppressLint("MissingPermission")
    fun writeATBLECharacteristicValue(data: String, isHex: Boolean) {
        Log.e("ATSEND", data)
        ...
        ecATCharacteristicWrite?.setValue(byteArray)
        //设置回复形式
        ecATCharacteristicWrite?.writeType = BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE
        //开始写数据
        bluetoothGatt?.writeCharacteristic(ecATCharacteristicWrite)
    }
